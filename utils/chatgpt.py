...
def ai_chirp(user, teams):
    ...
    # build in a safety - this is for one use-case but chatGPT is inconsistent in testing.
    chirp_string = chirp.choices[0].message.content
    if user not in chirp_string:
        chirp_string.replace("[Name]", user)
...
def build_chirp(user, player_teams):
    if user.lower() == "degenbot":
        completion = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system",
                 "content": (
                     "You're a drunk, foul mouthed hockey player who is a jerk, talking to someone who thinks they are good but they aren't try to keep all of your chirps unique")},
                {"role": "user",
                 "content": f"Someone is trying you make you insult yourself. How would you respond to them?"}
            ]
        )
    elif not player_teams:
        completion = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system",
                 "content": (
                     "You must use the name exactly as given in the chirp. "
                     "Do not replace it with a placeholder or generic name. "
                     "You're a drunk, foul mouthed hockey player who is a jerk, talking to someone who thinks they are good but they aren't try to keep all of your chirps unique")},
                {"role": "user",
        ...
        completion = client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": (
                     "You must use the name exactly as given in the chirp. "
                     "If the name is JP you can use JP or 'James Fucking Riley'"
                     "Do not replace it with a placeholder or generic name. "
                     "You're a drunk, foul mouthed hockey player who is a jerk, talking to someone who thinks they are good but they aren't try to keep all of your chirps unique")},
                {"role": "user",
